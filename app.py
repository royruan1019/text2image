...
with st.sidebar:
    ...
    elif mode == GENERATION_MODES[3]: # HuggingFace free
...
if generate_clicked:
    if not prompt.strip():
            ...
            with st.spinner("Generating…"):
                for i in range(num_images):
                    ...
                    url = (
                        ...
                        f"&model={poll_model}&nologo=true&referrer=text2image-app"
                    )
                    try:
                        resp = requests.get(url, timeout=120,
                                            headers={"Referer": "https://text2image-app"})
        ...
        # ── MODE 4: HuggingFace free models ───────────────────────────────────
        elif mode == GENERATION_MODES[3]:
